...
type CountryHandler struct {
	Usecase um.CountryUsecaseItf
	conf    *general.SectionService
	log     *logrus.Logger
}

func newCountryHandler(uc usecase.Usecase, conf *general.SectionService, logger *logrus.Logger) CountryHandler {
	return CountryHandler{
		Usecase: uc.Master.Country,
		conf:    conf,
		log:     logger,
	}
}

func (ch CountryHandler) GetListCountry(res http.ResponseWriter, req *http.Request) {
	respData := &handlers.ResponseData{
		Status: cg.Fail,
	}

	var tableFilter dm.CountryFilter
	var err error

	paginationData := general.GetPagination()

	// Check city name value
	...
	// Check orderby value.
	paginationData.OrderBy = null.StringFrom("country_id")
	...
	data, paginationData, _, err := ch.Usecase.GetListCountry(paginationData, tableFilter)
	if err != nil {
		respData.Message = "fail to get list country"
	...
	respData = &handlers.ResponseData{
		Status:  cg.Success,
		Message: "success get list country",
