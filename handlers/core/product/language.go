...
type LanguageHandler struct {
	Usecase um.LanguageUsecaseItf
	conf    *general.SectionService
	log     *logrus.Logger
}

func newLanguageHandler(uc usecase.Usecase, conf *general.SectionService, logger *logrus.Logger) LanguageHandler {
	return LanguageHandler{
		Usecase: uc.Product.Language,
		conf:    conf,
		log:     logger,
	}
}

func (dh LanguageHandler) GetListLanguage(res http.ResponseWriter, req *http.Request) {
	respData := &handlers.ResponseData{
		Status: cg.Fail,
	}

	var tableFilter dm.LanguageFilter
	var err error
	...
	// Check orderby value.
	paginationData.OrderBy = null.StringFrom("language_id")
	...
	data, paginationData, _, err := dh.Usecase.GetListLanguage(paginationData, tableFilter)
	if err != nil {
		respData.Message = "fail to get list language"
	...
	respData = &handlers.ResponseData{
		Status:  cg.Success,
		Message: "success get list language",
